...
main = do
    content <- get urlTest
    let body = content ^. responseBody
    let doc = parseHtml $ TL.unpack $ TLE.decodeUtf8 body
    let z = runX (doc >>> css "script" >>> Text.XML.HXT.Core.deep getText) >>= pure . map T.pack >>= pure . filter (T.isPrefixOf ytInfoPrefix)
    a <- z
    case a of
